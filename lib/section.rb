class MyStudy  #{{{1
  ...
  def design_detail_info(dd_id)  #{{{2
    ...
    if design_detail.field_type == "text"
      ...
      ## Matrix select falls under this category
      _build_dd_matrix_checkbox(dd_id)
    elsif design_detail.field_type.include?('matrix_radio')
      ## Matrix radio falls under this category
      _build_dd_matrix_radio(dd_id)
    elsif design_detail.field_type.include?('matrix')
      ## Matrix radio falls under this category
      _build_dd_matrix(dd_id)
    else
  ...
  def _build_dd_matrix(dd_id)  #{{{2
    ## This cannot return nil (I think)
    ddf_rows = DesignDetailField.find_all_by_design_detail_id_and_column_number(dd_id, 0)
    ddf_cols = DesignDetailField.find_all_by_design_detail_id_and_row_number(dd_id, 0)

    ## find_dddp_matrix also takes care of printing the csv line
    find_dddp_matrix(dd_id, ddf_rows, ddf_cols)
  end
  ...
  def _build_dd_one_column(dd_id)  #{{{2
    ...
    if field_type == "checkbox"
      ## A flag to let us know if this particular design detail checkbox question has had any answers
      any_choice_selected = false

      ## We will collect all datapoints that are found for each choice in this array
      found_datapoints = Array.new
      ...
      ddf.each do |d|
        ...
        _print_dd_csv(dd_id, dddp)
#        unless dddp.blank?
#          any_choice_selected = true
#          found_datapoints.push dddp
#        end
      end

#      if any_choice_selected
#        found_datapoints.each do |dddp|
#          _print_dd_csv(dd_id, dddp)
#        end
#      else
...
#      end
      ...
      if row.row_number == -1
        ...
        ddf_cols.each do |col|
          dddp = DesignDetailDataPoint.find_by_design_detail_field_id_and_value_and_study_id_and_extraction_form_id_and_row_field_id(dd_id,
                                                                                                                                     ...
                                                                                                                                     row.id)
          unless dddp.blank?
            answered_any_in_row = true
            found_datapoints.push dddp
          end
        end

        if answered_any_in_row
          found_datapoints.each do |dddp|
            _print_dd_csv(dd_id, dddp)
          end
        else
  ...
  def find_dddp_matrix(dd_id, ddf_rows, ddf_cols)
    ddf_rows.each do |row|
      if row.row_number == -1
        dddp = DesignDetailDataPoint.find_by_design_detail_field_id_and_study_id_and_extraction_form_id_and_row_field_id(dd_id,
                                                                                                                         @study_id,
